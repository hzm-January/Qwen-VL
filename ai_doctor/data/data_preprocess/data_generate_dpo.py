...
save_dpo_dataset_path = file_pathes['save_dpo_dataset']
...
def create_train_test_dataset_dpo(args):
    ...
    for i, patient_info in enumerate(org_train_dataset):
        ...
        label = train_data_labels[i]
        patient_description = {'id': str(uuid.uuid4()),
                               'chosen': [{'content': user_value, 'role': 'user'},
                                          {'content': 'Yes' if label else 'No', 'role': 'assistant'}],
                               'rejected': [{'content': user_value, 'role': 'user'},
                                            {'content': 'No' if label else 'Yes', 'role': 'assistant'}],
                               'prompt': user_value
                               # 'type': 'stage3'
                               }
...
def main(args):
    ...
    train_dataset, test_dataset, label_info = create_train_test_dataset_dpo(args)
    # print(train_dataset[0], '\n', test_dataset[0], '\n', label_info[0])
    with open(save_dpo_dataset_path + file_names['dpo_finetune_dataset_json'], 'w') as f:
        for row in train_dataset:
            row = json.dumps(row, ensure_ascii=False)
            f.write(row + '\n')
    with open(save_dpo_dataset_path + file_names['dpo_test_dataset_json'], 'w') as f:
        json.dump(test_dataset, f, ensure_ascii=False)
    with open(save_dpo_dataset_path + file_names['dpo_test_label_json'], 'w') as f:
